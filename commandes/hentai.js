...
async (origineMessage, zk, commandeOptions) => {
  ...
  if (!verifGroupe && !superUser) {
    repondre("Ce commande est réservée aux groupes uniquement. 🙄");
    return;
  }
  ...
  if (!isHentaiGroupe && !superUser) {
    repondre("Ce groupe ne fait pas partie de la liste des groupes 'pervers'. 🤭 Calme-toi !");
    return;
  }
  ...
  try {
    ...
    repondre('Une erreur est survenue lors de la récupération des données. 😓 ' + error);
  }
});
...
async (origineMessage, zk, commandeOptions) => {
  ...
  if (!verifGroupe && !superUser) {
    repondre("Cette commande est réservée aux groupes uniquement. 🙄");
    return;
  }
  ...
  if (!isHentaiGroupe && !superUser) {
    repondre("Ce groupe n'est pas dans la liste des groupes 'pervers'. 🤭 Calme-toi !");
    return;
  }
...
async (origineMessage, zk, commandeOptions) => {
  ...
  try {
    ...
    zk.sendMessage(origineMessage, { video: { url: videos[i].video_1 }, caption: `*Titre :* ${videos[i].title} \n *Catégorie :* ${videos[i].category}` }, { quoted: ms });
  } catch (error) {
    repondre('Une erreur est survenue lors de la récupération des vidéos. 😓 ' + error);
  }
});
