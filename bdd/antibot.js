...
// Récupérez l'URL de la base de données à partir de la configuration
...
async function createAntibotTable() {
  ...
  try {
    ...
    `);
    console.log("La table 'antibot' a été créée avec succès. Un autre succès pour DRK_ST_TROLL !");
  } catch (error) {
    console.error("Erreur lors de la création de la table 'antibot':", error);
  } finally {
    client.release();
  }
}
...
// Fonction pour ajouter ou mettre à jour un enregistrement dans la table "antibot"
async function atbajouterOuMettreAJourJid(jid, etat) {
  const client = await pool.connect();
  try {
    // Vérifiez si le jid existe déjà dans la table 'antibot'
    ...
    console.log(`JID ${jid} ajouté ou mis à jour avec succès dans la table 'antibot'. DRK_ST_TROLL est en contrôle.`);
  } catch (error) {
    console.error("Erreur lors de l'ajout ou de la mise à jour du JID dans la table 'antibot':", error);
  } finally {
    client.release();
  }
}

// Fonction pour mettre à jour l'action pour un JID spécifique
async function atbmettreAJourAction(jid, action) {
  ...
  try {
    ...
    console.log(`Action mise à jour avec succès pour le JID ${jid} dans la table 'antibot'. T'as bien trollé !`);
  } catch (error) {
    console.error("Erreur lors de la mise à jour de l'action pour le JID dans la table 'antibot':", error);
  } finally {
    client.release();
  }
}

// Fonction pour vérifier l'état d'un JID spécifique
async function atbverifierEtatJid(jid) {
  ...
  try {
    ...
    console.error("Erreur lors de la vérification de l'état du JID dans la table 'antibot':", error);
    return false;
  } finally {
    client.release();
  }
...
// Fonction pour récupérer l'action d'un JID spécifique
async function atbrecupererActionJid(jid) {
  ...
  try {
    ...
    console.error("Erreur lors de la récupération de l'action du JID dans la table 'antibot':", error);
